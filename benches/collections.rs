...
fn vec_find(data: &Vec<(usize, Vec<u8>)>, id: usize)  {
    if let Some(_)=data.iter().find(|(i, _)| i == &id){
        return;
    }
}
...
fn vec_vs_hashmap(c: &mut Criterion) {
    ...
    for &data_size in &data_sizes {
        ...
        for data_len in data_lengths{
            ...
            let hashmap_data = (0..data_len).map(|i| (i, data.clone())).collect::<HashMap<_, _>>();

            //  we create a btree of data
            let btree_data = (0..data_len).map(|i| (i, data.clone())).collect::<BTreeMap<_, _>>();

            //  we create a dashmap of data
            let dashmap_data = (0..data_len).map(|i| (i, data.clone())).collect::<DashMap<_, _>>();

            //  we create a indexmap of data
            let indexmap_data = (0..data_len).map(|i| (i, data.clone())).collect::<IndexMap<_, _>>();

            //  we create a smallvec of data
            let smallvec_1_data = (0..data_len).map(|i| (i, data.clone())).collect::<SmallVec<[(usize, Vec<u8>); 1]>>();
            let smallvec_4_data = (0..data_len).map(|i| (i, data.clone())).collect::<SmallVec<[(usize, Vec<u8>); 4]>>();
            let smallvec_8_data = (0..data_len).map(|i| (i, data.clone())).collect::<SmallVec<[(usize, Vec<u8>); 8]>>();
            let smallvec_16_data = (0..data_len).map(|i| (i, data.clone())).collect::<SmallVec<[(usize, Vec<u8>); 16]>>();
            let smallvec_32_data = (0..data_len).map(|i| (i, data.clone())).collect::<SmallVec<[(usize, Vec<u8>); 32]>>();

            //  we create a vecdeque of data
            let vecdeque_data = (0..data_len).map(|i| (i, data.clone())).collect::<VecDeque<_>>();

            //  we create a fxhashmap of data
            let fxhashmap_data = (0..data_len).map(|i| (i, data.clone())).collect::<FxHashMap<_, _>>();


            //  we create a benchmark for the vector
            group.bench_with_input(
                BenchmarkId::new("Vec Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
            ...
            group.bench_with_input(
                BenchmarkId::new("HashMap Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
            ...
            group.bench_with_input(
                BenchmarkId::new("BTreeMap Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
            ...
            group.bench_with_input(
                BenchmarkId::new("DashMap Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
            ...
            group.bench_with_input(
                BenchmarkId::new("IndexMap Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
            ...
            group.bench_with_input(
                BenchmarkId::new("SmallVec<1> Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
                &(smallvec_1_data, index_to_find),
                |b, (smallvec_1_data, index_to_find)| {
                    b.iter(|| smallvec_find_1(black_box(smallvec_1_data), black_box(*index_to_find)));
                });

            group.bench_with_input(
                BenchmarkId::new("SmallVec<4> Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
                &(smallvec_4_data, index_to_find),
                |b, (smallvec_4_data, index_to_find)| {
                    b.iter(|| smallvec_find_4(black_box(smallvec_4_data), black_box(*index_to_find)));
                });
            group.bench_with_input(
                BenchmarkId::new("SmallVec<8> Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
                &(smallvec_8_data, index_to_find),
                |b, (smallvec_8_data, index_to_find)| {
                    b.iter(|| smallvec_find_8(black_box(smallvec_8_data), black_box(*index_to_find)));
                });
            group.bench_with_input(
                BenchmarkId::new("SmallVec<16> Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
                &(smallvec_16_data, index_to_find),
                |b, (smallvec_16_data, index_to_find)| {
                    b.iter(|| smallvec_find_16(black_box(smallvec_16_data), black_box(*index_to_find)));
                });
            group.bench_with_input(
                BenchmarkId::new("SmallVec<32> Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
                &(smallvec_32_data, index_to_find),
                |b, (smallvec_32_data, index_to_find)| {
                    b.iter(|| smallvec_find_32(black_box(smallvec_32_data), black_box(*index_to_find)));
                });
            //  we create a benchmark for the vecdeque
            group.bench_with_input(
                BenchmarkId::new("VecDeque Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
            ...
            group.bench_with_input(
                BenchmarkId::new("FxHashMap Find", format!("Size per el = {} KB, Number of el = {data_len}",data_size/1024)),
